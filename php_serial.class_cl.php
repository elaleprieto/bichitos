...
class phpSerial
{
	...
	function deviceSet ($device)
	{
		if ($this->_dState !== SERIAL_DEVICE_OPENED)
		{
			if ($this->_os === "linux")
			{
				...
				if (preg_match("@^ACM(\d+):?$@i", $device, $matches))
				{
					$device = "/dev/ttyACM" . $matches[1];
				}

				if ($resultado = $this->_exec("stty -F " . $device) === 0)
				{
					$this->_device = $device;
			...
			trigger_error("Specified serial port is not valid :: " . $device . " " . $resultado, E_USER_WARNING);
			return false;
		}
		else
		{
	...
	function _ckOpened()
	{
		if ($this->_dState !== SERIAL_DEVICE_OPENED)
		{
			trigger_("Device must be opened", E_USER_WARNING);
			return false;
		}
